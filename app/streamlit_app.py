...
if should_generate:
    ...
    if gen_mode == 'Single Event':
        ...
        for i, event in enumerate(events):
            ...
            # --- NEW: Log to Supabase (using rule-based explanation for now) ---
            if prediction['is_anomaly']:
...
# ============================================================================
# TAB 2: HISTORICAL DATABASE TRENDS (NEW)
# ============================================================================
with tab_history:
    if not supabase:
        ...
        if not db_data:
            ...
            hc2.metric("Most Common Trigger", mode_trigger)
            
            critical_count = len(df_history[df_history['severity'] == 'CRITICAL'])
            hc3.metric("Critical Anomalies", critical_count)
