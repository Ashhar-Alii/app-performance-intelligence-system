...
# ============================================================================
# COOKIE CONTROLLER — Must be initialized before any session state reads
# ============================================================================
...
def init_session_state():
    ...
    # FIX 1: Track whether we just logged out so the cookie isn't re-applied
...
# FIX 1: Only restore from cookie if we did NOT just log out this run.
# This prevents the cookie from immediately re-logging the user in after
# a secure logout (which deletes the cookie but reruns the script).
# ============================================================================
if st.session_state.user_email is None and not st.session_state.just_logged_out:
    saved_email = cookie_controller.get('user_email')
    if saved_email:
        st.session_state.user_email = saved_email

# Reset the just_logged_out flag after we've safely skipped the cookie restore
...
if st.session_state.user_email is None:
    ...
    if st.session_state.lockout_until:
        if datetime.now() < st.session_state.lockout_until:
            ...
            st.error(f"🔒 Account temporarily locked due to multiple failed attempts. Try again in {remaining} seconds.")
            st.stop()
        else:
            st.session_state.login_attempts = 0
            st.session_state.lockout_until = None
    ...
    with col2:
        ...
        with tab_login:
            ...
            with c2:
                # ──────────────────────────────────────────────────────────
                # FIX 2: Password reset — strip + lowercase the email before
                #         querying, so trailing spaces / mixed case don't
                #         cause a false "no account found" error.
                # ──────────────────────────────────────────────────────────
                with st.popover("Forgot Password?"):
                    ...
                    if st.button("Update Password", key="rec_btn", type="primary"):
                        if rec_email and new_pass and supabase:
                            # Always clean the email before any DB operation
                            clean_rec_email = rec_email.strip().lower()

                            if not is_valid_gmail(clean_rec_email):
...
with st.sidebar:
    ...
    if st.button("🚪 Secure Logout", use_container_width=True):
        # FIX 1: Set the flag BEFORE clearing session state so the cookie
        #         restore block at the top is skipped on the next rerun.
        ...
        # FIX 1: Use .delete() — the correct method for streamlit_cookies_controller.
        #         .remove() does not exist and silently fails, leaving the cookie alive.
        try:
            cookie_controller.delete('user_email')
        except Exception:
    ...
    if st.session_state.current_result:
                    ...
                    if is_anomaly and st.session_state.current_result.get('contributions'):
                        ...
                        if causal_chain:
                            for step in causal_chain:
                                s_icon = "🔴" if step['step'] == len(causal_chain) else "⚡" if step['step'] == 1 else "→"
        ...
        if len(st.session_state.history) > 5:
            ...
            with st.expander("📋 Click to Expand Verification Details", expanded=False):
                ...
                st.markdown("Each event is **generated** as either normal or anomalous. The model **independently** predicts whether it's an anomaly.")
        ...
        for col, (icon, title, desc) in zip(feat_cols, features_info):
            with col:
