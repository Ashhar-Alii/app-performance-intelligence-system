...
# ============================================================================
# LOGIN WALL (Stops execution if not logged in)
# ============================================================================
    ...
    with col2:
        ...
        with tab_login:
            ...
            if st.button("Login", use_container_width=True, type="primary"):
                if log_user and log_pass and supabase:
                    ...
                    st.warning("Please enter credentials and ensure DB is connected.")
        ...
        with tab_signup:
            ...
            if st.button("Sign Up", use_container_width=True):
                if reg_user and reg_pass and supabase:
...
def log_anomaly_to_db(event, prediction, explanation=None, event_type="unknown"):
    ...
    try:
        ...
        data = {
            "username": st.session_state.username, # <--- NEW: Links to specific user
            "is_anomaly": True,
...
if should_generate:
    ...
    if gen_mode == 'Single Event':
        ...
        # Single event → use LLM (only 1 API call)
        ...
        # Update explainer in session state
        st.session_state.explainer = explainer
        
        # --- NEW: DB LOGGING FOR LIVE SINGLE GENERATION ---
        ...
        # Batch generation — DON'T call LLM for every event
        ...
        for i, event in enumerate(events):
            ...
            # --- NEW: DB LOGGING FOR BATCH GENERATION ---
            if prediction['is_anomaly']:
        ...
        # Now call LLM ONCE for the last anomaly event only
        if last_anomaly_index is not None and explainer.llm_available:
            ...
            # Also update the DB record to have the LLM explanation instead of rule-based
            # (Note: we log it once initially, updating is complex so we just leave DB as rule-based for now)
...
with tab_history:
    if not supabase:
        ...
        with st.spinner("Fetching your historical anomalies from PostgreSQL..."):
            try:
                # --- NEW: Filters database using the specific username! ---
        ...
        if not db_data:
            st.info(f"No anomalies saved for user **{st.session_state.username}** yet. Generate anomalies in the Live Dashboard!")
        else:
            ...
            # Clean up the dataframe view for the user
