...
def init_session_state():
    defaults = {
        ...
        'login_attempts': 0,
        'lockout_until': None,
        # FIX 1A — tracks whether logout was just performed so we skip
        # the cookie-restore block on the very next rerun
...
# ============================================================================
# COOKIE CONTROLLER
# Must be instantiated AFTER session state init and BEFORE any cookie reads.
# Using a fixed key so Streamlit reuses the same component across reruns.
# ============================================================================
cookie_controller = CookieController(key='cc')

# ============================================================================
# FIX 1B — Cookie to session restore
#
# The CookieController caches cookies in st.session_state['cc'].
# When .remove() is called, it sends the remove command to the browser
# but the session state cache still holds the old value.
# We manually wipe the cache entry on logout so the very next rerun
# does not restore the email from the stale cache.
# ============================================================================
    ...
    with col2:
        ...
        with tab_login:
            ...
            with c2:
                ...
                # Use unique widget keys that don't clash with variable names.
                # Read values from st.session_state directly so Streamlit
                # never loses the value when the popover re-renders.
                # Always .strip().lower() before any DB query.
                # ──────────────────────────────────────────────────────────
                with st.popover("Forgot Password?"):
                    ...
                    if st.button("Update Password", key="rec_btn", type="primary"):
                        ...
                        elif not is_valid_gmail(clean_rec_email):
                            st.error("Please enter a valid @gmail.com address.")
                        elif len(raw_new_pass) < 8:
            ...
            if st.button("Log In", use_container_width=True, type="primary"):
                if log_email and log_pass and supabase:
                    ...
                    if res.data and res.data[0]['password_hash'] == hash_password(log_pass):
                        ...
                        st.session_state.login_attempts = 0
                        if remember_me:
                            cookie_controller.set('user_email', clean_email,
                                                  max_age=30 * 86400)
...
def log_anomaly_to_db(event, prediction, explanation=None, event_type="unknown"):
    ...
    try:
        ...
        supabase.table("anomaly_logs").insert({
...
with st.sidebar:
    ...
    if st.button("🚪 Secure Logout", use_container_width=True):
        # FIX 1C — Set the flag BEFORE rerun so the cookie-restore block
        ...
        # Clear all app state
        for key in ['user_email', 'history', 'current_result',
                    'current_event', 'current_explanation', 'auto_generate']:
            st.session_state[key] = [] if key == 'history' else \
                                    False if key == 'auto_generate' else None
    ...
    if ('explainer_config' not in st.session_state or
        ...
        explainer = AIExplainer(provider=ai_provider,
    ...
    elif ai_status.get('llm_available'): st.info(ai_status['status'])
    elif ai_provider == 'rule_based':    st.info(ai_status['status'])
...
if st.session_state.get('analyze_csv') and st.session_state.get('uploaded_csv'):
    ...
    try:
        ...
        st.caption(f"Analyzing {len(df_upload)} rows…")

        csv_results = []
        for idx, row in df_upload.iterrows():
            row_dict = row.to_dict()
            features = {k: float(v) for k, v in row_dict.items()
            ...
            if pred['is_anomaly']:
                mock_event = {'features': features, 'raw_display': features,
        ...
        if csv_results:
            ...
            st.download_button("📥 Download Results CSV",
                               ...
                               "csv_analysis_results.csv", "text/csv",
...
if should_generate:
    ...
    if gen_mode == 'Single Event':
        ...
        if last_anomaly_index is not None and explainer.llm_available:
            la  = st.session_state.history[last_anomaly_index]
            llm = explainer.explain(la['prediction'], la['event'], use_llm=True)
            st.session_state.history[last_anomaly_index]['explanation'] = llm
            st.session_state.explainer = explainer
        ...
        if events:
            ...
            st.session_state.current_explanation = (
                ...
                if last_anomaly_index is not None else last['explanation']
            )
...
st.markdown('<p style="text-align:center;color:#666;margin-top:-10px;">Real-Time Mobile App Performance Monitoring & AI-Powered Analysis</p>',
            unsafe_allow_html=True)
st.markdown("")
...
with tab_live:
    if st.session_state.current_result:
        ...
        if (len(st.session_state.history) > 10 and
                st.session_state.current_result.get('contributions')):
            st.markdown("---")
            st.markdown("### 📊 Feature Distribution Analysis")
            top_3 = st.session_state.current_result['contributions'][:3]
            for col, feat in zip(st.columns(len(top_3)), top_3):
                with col:
                    render_feature_histogram(feat['value'], feat['feature'],
                                             st.session_state.history)
            ...
            with st.expander(f"{icon} AI Root Cause Analysis", expanded=is_anomaly):
                ...
                if is_anomaly:
                    ...
                    if contributions:
                        ...
                        } for f in contributions]), use_container_width=True, hide_index=True)
                    ...
                    if causal_chain:
                        for step in causal_chain:
                            s_icon = ("🔴" if step['step'] == len(causal_chain)
                                      else "⚡" if step['step'] == 1 else "→")
                            st.markdown(f"**{s_icon} Step {step['step']}** ({step['component']}): {step['event']}")
                    ...
                    if recovery:
                        ...
                        rc1.metric("📈 Expected Improvement",  f"{recovery['improvement_percent']}%",  "metric recovery")
                        rc2.metric("⏱️ Est. Recovery Time",    recovery['estimated_recovery_time'])
                        rc3.metric("🎯 Confidence",            recovery['confidence'])

                        import plotly.graph_objects as go
                        feats = list(recovery['current_state'].keys())
                        fig = go.Figure(data=[
                            go.Bar(name='Current (Anomalous)',   x=feats,
                                   y=list(recovery['current_state'].values()),   marker_color='#FF4B4B'),
                            go.Bar(name='After Fix (Projected)', x=feats,
                                   y=list(recovery['recovered_state'].values()), marker_color='#40C057'),
                        ])
                        fig.update_layout(title='Before vs After Recovery (Z-Score Deviation)',
        ...
        if len(st.session_state.history) > 5:
            ...
            st.markdown("*Proves the model is genuinely detecting anomalies, not randomly guessing.*")
            ...
            with st.expander("📋 Click to Expand Verification Details", expanded=False):
                ...
                s1.metric("✅ Correctly Detected", f"{tp+tn}", f"{(tp+tn)/total*100:.1f}% accuracy")
                s2.metric("🎯 True Positives",     str(tp),    "Anomalies correctly caught")
                s3.metric("❌ False Positives",    str(fp),    "Normal wrongly flagged")
                s4.metric("⚠️ Missed Anomalies",   str(fn),    "Anomalies not caught")

                if generated_anomalies > 0:
                    dr  = tp / generated_anomalies * 100
                    bg  = '#1B5E20' if dr > 70 else '#B71C1C'
                    st.markdown(f"""<div style="background:{bg};padding:1rem;border-radius:8px;margin:0.5rem 0;"><b style="color:white;">📊 Anomaly Detection Rate: {dr:.1f}%</b><br><span style="color:#F0F0F0;">Out of {generated_anomalies} generated anomalies, {tp} correctly detected. {'✅ Good performance!' if dr>70 else '⚠️ Some anomalies were subtle and missed.'}</span></div>""", unsafe_allow_html=True)

                if generated_normal > 0 and detected_anomalies > 0:
                    pr  = tp / detected_anomalies * 100
                    bg2 = '#1B5E20' if pr > 50 else '#B71C1C'
                    st.markdown(f"""<div style="background:{bg2};padding:1rem;border-radius:8px;margin:0.5rem 0;"><b style="color:white;">🎯 Alert Precision: {pr:.1f}%</b><br><span style="color:#F0F0F0;">Out of {detected_anomalies} alerts, {tp} were actual anomalies. {'✅ Reliable alerts!' if pr>50 else '⚠️ Some false alarms present.'}</span></div>""", unsafe_allow_html=True)
                ...
                if normal_scores and anomaly_scores:
                    sf = go.Figure()
                    sf.add_trace(go.Box(y=normal_scores,  name='Generated Normal',
                                        marker_color='#40C057', boxmean=True))
                    sf.add_trace(go.Box(y=anomaly_scores, name='Generated Anomaly',
                                        marker_color='#FF4B4B', boxmean=True))
                    sf.add_hline(y=50, line_dash="dash", line_color="black",
                                 annotation_text="Detection Threshold (50%)")
                    sf.update_layout(title='Score Distribution: Normal vs Anomaly',
                                     yaxis_title='Anomaly Score (%)', height=350)
                    st.plotly_chart(sf, use_container_width=True)
                    avg_n = np.mean(normal_scores)
                    avg_a = np.mean(anomaly_scores)
                    gap   = abs(avg_a - avg_n)
                    st.markdown(f"""<div style="background:#1E3A5F;padding:1rem;border-radius:8px;"><b style="color:white;">📊 Score Analysis:</b><br><span style="color:#A8C8E8;">• Normal avg: </span><b style="color:#69DB7C;">{avg_n:.1f}%</b><br><span style="color:#A8C8E8;">• Anomaly avg: </span><b style="color:#FF6B6B;">{avg_a:.1f}%</b><br><span style="color:#A8C8E8;">• Gap: </span><b style="color:white;">{gap:.1f}% {'✅ Good separation!' if gap>15 else '⚠️ Moderate separation'}</b></div>""", unsafe_allow_html=True)
        ...
        if st.session_state.history:
            ...
            st.download_button("📥 Download Predictions (CSV)",
                               pd.DataFrame(download_data).to_csv(index=False),
