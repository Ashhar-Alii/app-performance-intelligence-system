...
if st.session_state.user_email is None:
    ...
    if saved_email and saved_email.strip():  # ← add .strip() guard
...
if st.session_state.user_email is None:
    if (
        ...
        and datetime.now() < st.session_state.lockout_until
    ):
        st.error(
            f"🔒 Account temporarily locked. Try again in {int((st.session_state.lockout_until - datetime.now()).total_seconds())} seconds."
        )
        st.stop()
        ...
        with tab_login:
            ...
            with st.expander("Forgot Password?"):
                ...
                rec_email = st.text_input(
                    "Confirm your Gmail Address", key="rec_email_input"
                )
                ...
                if st.button("Update Password", key="rec_btn", type="primary"):
                    rec_email_val = (
                        st.session_state.get("rec_email_input", "").strip().lower()
                    )
                    ...
                    if not rec_email_val or not new_pass_val:
                        ...
                        res = (
                            ...
                            .select("email")
                            .eq("email", rec_email_val)
                            .execute()
                        )
                        if len(res.data) > 0:
                            if len(new_pass_val) >= 8:
                                supabase.table("app_users").update(
                                    {"password_hash": hash_password(new_pass_val)}
...
with st.sidebar:
    ...
    if force_anomaly:
        ...
        st.info(
            f"{ANOMALY_DESCRIPTIONS[anomaly_type]['icon']} **{ANOMALY_DESCRIPTIONS[anomaly_type]['name']}**\n\n{ANOMALY_DESCRIPTIONS[anomaly_type]['description']}"
        )
    else:
    ...
    elif ai_status.get("llm_available") or ai_provider == "rule_based":
...
if should_generate:
    ...
    if gen_mode == "Single Event":
        ...
        if events:
            ...
            st.session_state.current_result, st.session_state.current_event = (
                last["prediction"],
                last["event"],
            )
